<?php  
   ...
   define('CAPTCHA_WIDTH',        180); // max 500
   define('CAPTCHA_HEIGHT',        22); // max 200
   define('CAPTCHA_NUM_CHARS',      5);
   define('CAPTCHA_NUM_LINES',     30);
   define('CAPTCHA_MIN_FONT_SIZE', 14);
   define('CAPTCHA_MAX_FONT_SIZE', 20);
   define('CAPTCHA_FONT_ANGLE',    15); // Try 15 if not readable
	
   define('BG_COLOR_RED',         255);
   ...
   class PhpCaptcha {
      ...
      function DrawCharacters() {
         ...
         for ($i = 0; $i < strlen($this->sCode); $i++) {
            ...
            // select random angle
            $iAngle = rand(-1*(CAPTCHA_FONT_ANGLE), CAPTCHA_FONT_ANGLE);
            ...
            if ($this->bCharShadow) {
               $iOffsetAngle = rand(-1*(CAPTCHA_FONT_ANGLE), CAPTCHA_FONT_ANGLE);
      ...
      function Create($sFilename = '') {
         ...
         imagecolorallocate($this->oImage, BG_COLOR_RED, BG_COLOR_GREEN, BG_COLOR_BLUE);
