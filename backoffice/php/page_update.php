...
{     
	  $id= $_POST['id'];
	  $previous_page=f(q("select * from general_pages where id=$id",$dbconnect));
	  ...
	  $position1 = str_replace("'","&rsquo;",$_POST['txtposition']);
	  $position = strtolower(str_replace("’","&rsquo;",$position1));
	  
	 //$page_title = htmlentities($_POST['page_title'], ENT_QUOTES);
...
if($id == 0) 
{
	$sql = "insert into general_pages(id,page_title,page_subtitle,url,browser_title,intro_text,url_title, meta_keywords,meta_description,picture,image_alt,pdf,listing_title,description,position,status) values('','$page_title','$page_subtitle','$url','$browser_title','$intro_text','$url_title','$meta_keywords','$meta_description','$imgname','$image_alt','$pdfname','$listing_title','$description','$position','1')";
} else 
{
			if($_FILES['picture']['name']!="" && $_FILES['pdf']['name']!="")
			{
			echo   $sql = "update general_pages set page_title='$page_title', page_subtitle='$page_subtitle', url='$url', browser_title='$browser_title', intro_text='$intro_text', url_title='$url_title', meta_keywords='$meta_keywords', meta_description='$meta_description', picture='$imgname', image_alt='$image_alt', pdf='$pdfname', listing_title='$listing_title', description='$description', position='$position' where id='$id'";
			}
			else if($_FILES['picture']['name']!="" && $_FILES['pdf']['name']=="")
			{
			   $sql = "update general_pages set page_title='$page_title', page_subtitle='$page_subtitle', url='$url', browser_title='$browser_title', intro_text='$intro_text', url_title='$url_title', meta_keywords='$meta_keywords', meta_description='$meta_description', picture='$imgname', image_alt='$image_alt', listing_title='$listing_title', description='$description', position='$position' where id='$id'";
			}
			else if($_FILES['picture']['name']=="" && $_FILES['pdf']['name']!="")
			{
			   $sql = "update general_pages set page_title='$page_title', page_subtitle='$page_subtitle', url='$url', browser_title='$browser_title', intro_text='$intro_text', url_title='$url_title', meta_keywords='$meta_keywords', meta_description='$meta_description', image_alt='$image_alt', pdf='$pdfname', listing_title='$listing_title', description='$description', position='$position' where id='$id'";
			}
			else
			{
...
if(q($sql,$dbconnect))
{
	...
	if($id=="" || $id==0)
	{
		...
		$hurl_name="general_page.php";
		$sql_inst = "insert into tbl_page_file(id,page_id,page_file_name,parameter1)values('','$new_id','$hurl_name','pageid')";
...
?>
 <script language="JavaScript">
  window.location="index.php?page=page_list&msg=<?php echo $msg?>";
 </script>
...
?>  
 <script language="JavaScript">
  window.location="index.php?page=page_list&msg_error=Page add failed.";
  </script>
<?php
 }
}
