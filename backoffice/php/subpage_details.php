...
if(isset($_GET['dimg']))
{
	$sel_img = "select picture from general_pages where parent_id=$pageid and id='$_GET[dimg]'";
	...
	$id = $_GET['id'];
	$sql = "select * from general_pages where parent_id=$pageid and id = $id";
	$qry = q($sql,$dbconnect);
	$result = f($qry);
	...
	if($result['page_order']==''){
		$order = 0;
	}else{
		$order = $result['page_order'];
	}
	$url = $result['url'];
  ...
  if($result['picture']!="" && file_exists("../general_image/thumb0/$result[picture]"))
  {
	...
	include $htmlfile."subpage_details.html";
?>